        ...
        var e,
            ...
            c = {
                ...
                filterSlider: function () {
                    ...
                    if (e) {
                        noUiSlider.create(e, { 
                            start: [backendMinPrice, backendMaxPrice],
                            connect: true,
                            step: 100,
                            margin: 100,
                            range: { min: 0, max: backendMaxPrice }
                        });
                        ...
                        e.noUiSlider.on("update", function (values) {
                            ...
                            $("#filter-price-range").text(formatted.join(" - "));
                        });

                        // auto-submit after user stops sliding
                ...
                quantityInputs: function (e = "body") {
                    ...
                    t(e).find(".horizontal-quantity").TouchSpin({
                        ...
                        buttonup_txt: "",
                        buttondown_txt: "",
                        buttondown_class: "btn btn-outline btn-down-icon bootstrap-touchspin-down",
                        buttonup_class: "btn btn-outline btn-up-icon bootstrap-touchspin-up",
                        initval: 1,
                        min: 1
                    }).each(function () {
                        var input = t(this),
                            rowId = input.attr("data-rowid") || "unknown",
                            csrfToken = t('meta[name="csrf-token"]').attr("content") || "";

                        var touchSpinContainer = input.closest(".bootstrap-touchspin"),
                            reduceBtn = touchSpinContainer.find(".btn-down-icon.bootstrap-touchspin-down"),
                            increaseBtn = touchSpinContainer.find(".btn-up-icon.bootstrap-touchspin-up");
                        ...
                        function updateQuantity(url, input) {
                            t.ajax({
                                ...
                                success: function (response) {
                                    if (response.removed) {
                                        // Remove the entire product row
                                        input.closest("tr").remove();
                                    } else {
                                        if (response.qty !== undefined) {
                                            input.val(response.qty);
                                        }
                                        if (response.row_subtotal !== undefined) {
                                            input.closest("tr").find(".subtotal-price").text(response.row_subtotal);
                                            $('.cart-price').text('₦' + response.sub_total);
                                            $('.cart-count').text(response.count);
                                        }
                                    }
                                    if (response.cart_subtotal !== undefined) {
                                        t("#cart-subtotal").text(response.cart_subtotal);
                                        $('.cart-price').text('₦' + response.cart_subtotal);
                                        $('.cart-count').text(response.count)
                                    }
                                    if (response.cart_total !== undefined) {
                                        t("#cart-total").text(response.cart_subtotal);
                                    }
                                },
                                error: function (xhr) {
                        ...
                        // Attach AJAX calls to buttons
                        reduceBtn.on("click touchstart", function (event) {
                            event.preventDefault();
                            updateQuantity("/cart/reduce-quantity/" + rowId, input);
                        });

                        increaseBtn.on("click touchstart", function (event) {
                            event.preventDefault();
                            updateQuantity("/cart/increase-quantity/" + rowId, input);
                        });
                    });
                },
